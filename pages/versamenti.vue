        ...
        <v-card class="rounded-lg" elevation="5">
            ...
            <v-list v-else>
                <v-list-item two-line class="mb-5" v-for="item in pendingDeposit" :key="item.id">
                    <v-icon class="primary--text mr-5">mdi-cash-plus</v-icon>
                    <v-list-item-content>
                        <v-list-item-title>{{ item.name }}</v-list-item-title>
                        <v-list-item-subtitle>{{ item.date }}</v-list-item-subtitle>
                    </v-list-item-content>
                    <p class="mt-4 font-weight-bold warning--text">{{ item.credit }}€</p>
                </v-list-item>
            </v-list>
            ...
            <v-card-text>
                <v-list>
                    <v-list-item two-line v-for="item in deposits" :key="item.id" class="mb-5">
                        <v-icon class="mr-5 primary--text">mdi-cash-plus</v-icon>
                        <v-list-item-content>
                            <v-list-item-title>{{ item.name }}</v-list-item-title>
                            <v-list-item-subtitle>{{ item.date }}</v-list-item-subtitle>
                        </v-list-item-content>
